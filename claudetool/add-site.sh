...
echo "Step 6/7: Generating Terraform configuration..."

TERRAFORM_CONFIG="infrastructure/terraform/sites.tf"

# Generate Terraform snippet
cat >> "$TERRAFORM_CONFIG" <<TERRAFORM_EOF

# ${SITE_NAME_CAPITALIZED} - Auto-generated by add-site.sh
resource "google_artifact_registry_repository" "${SITE_NAME}_production" {
  location      = var.region
  repository_id = "${SITE_NAME}-production"
  description   = "Production Docker images for ${SITE_NAME_CAPITALIZED} site"
  format        = "DOCKER"
...
resource "google_artifact_registry_repository" "${SITE_NAME}_previews" {
  location      = var.region
  repository_id = "${SITE_NAME}-previews"
  description   = "Feature branch preview Docker images for ${SITE_NAME_CAPITALIZED}"
  format        = "DOCKER"
...
resource "google_artifact_registry_repository_iam_member" "${SITE_NAME}_production_writer" {
  project    = var.project_id
  location   = google_artifact_registry_repository.${SITE_NAME}_production.location
  repository = google_artifact_registry_repository.${SITE_NAME}_production.name
  role       = "roles/artifactregistry.writer"
  member     = "serviceAccount:\${data.google_service_account.github_actions.email}"
}

resource "google_artifact_registry_repository_iam_member" "${SITE_NAME}_previews_writer" {
  project    = var.project_id
  location   = google_artifact_registry_repository.${SITE_NAME}_previews.location
  repository = google_artifact_registry_repository.${SITE_NAME}_previews.name
  role       = "roles/artifactregistry.writer"
  member     = "serviceAccount:\${data.google_service_account.github_actions.email}"
}

output "${SITE_NAME}_production_registry" {
  value       = "\${var.region}-docker.pkg.dev/\${var.project_id}/\${google_artifact_registry_repository.${SITE_NAME}_production.repository_id}"
  description = "${SITE_NAME_CAPITALIZED} production Artifact Registry URL"
}

output "${SITE_NAME}_preview_registry" {
  value       = "\${var.region}-docker.pkg.dev/\${var.project_id}/\${google_artifact_registry_repository.${SITE_NAME}_previews.repository_id}"
  description = "${SITE_NAME_CAPITALIZED} preview Artifact Registry URL"
}
TERRAFORM_EOF

echo "✓ Generated Terraform configuration in infrastructure/terraform/sites.tf"

# ==================== Step 7: Summary ====================
echo "Step 7/7: Complete!"
echo ""
...
echo "  - infrastructure/terraform/sites.tf (Artifact Registry with cleanup policies)"
echo ""
echo "⚠️  NEXT STEPS:"
echo ""
echo "1. Run: npm install"
echo "2. Run: ./iac.py apply  (to create Artifact Registry repositories)"
