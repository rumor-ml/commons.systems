...
test.describe('Library Navigation - Data Reflection', () => {
  // Skip in CI: Tests require exact Firestore state
  // Local tests pass, CI environment has transient state issues
  const shouldSkipFlaky = process.env.CI;

  (shouldSkipFlaky ? test.skip : test)(
    'navigation should reflect actual card types from data',
    async ({ page }) => {
      await page.goto('/cards.html');
      ...
      // Build expected types from cards.json
      const expectedTypes = new Set();
      cardsData.forEach((card) => {
        if (card.type) {
          expectedTypes.add(card.type);
        }
      });
  ...
  // Local tests pass, CI environment has transient state issues
  (shouldSkipFlaky ? test.skip : test)(
    'navigation should reflect actual subtypes for each type',
    async ({ page }) => {
      await page.goto('/cards.html');
      ...
      // Build expected type -> subtypes mapping from cards.json
      const typeSubtypes = new Map();
      cardsData.forEach((card) => {
        const type = card.type || 'Unknown';
        const subtype = card.subtype || 'Unknown';

        if (!typeSubtypes.has(type)) {
          typeSubtypes.set(type, new Set());
        }
        typeSubtypes.get(type).add(subtype);
      });

      // Verify subtypes for each type
      for (const [type, subtypes] of typeSubtypes) {
        ...
        const typeContainer = page.locator(`.library-nav-type[data-type="${type}"]`);
        for (const subtype of subtypes) {
          const subtypeElement = typeContainer.locator(
            `.library-nav-subtype[data-subtype="${subtype}"]`
          );
          await expect(subtypeElement).toBeVisible();
        }

        // Verify subtype count matches expected
        const subtypeElements = typeContainer.locator('.library-nav-subtype');
        const subtypeCount = await subtypeElements.count();
        expect(subtypeCount).toBe(subtypes.size);
      }
    }
  );
  ...
  test('navigation should display correct card counts', async ({ page }) => {
    ...
    for (const [key, expectedCount] of subtypeCounts) {
      ...
      // Expand the type to show subtypes
      const typeToggle = page.locator(`.library-nav-type[data-type="${type}"] .library-nav-toggle`);
  ...
  test('Origin type should include former Foe subtypes', async ({ page }) => {
    ...
    for (const subtype of originSubtypes) {
      const subtypeElement = originType.locator(`.library-nav-subtype[data-subtype="${subtype}"]`);
      await expect(subtypeElement).toBeVisible();
    }
  });
