...
test.describe('HTMX Cross-Page Navigation', () => {
  ...
  const skipInCI = process.env.CI ? 'skip' : undefined;
  test(
  ...
  test('should navigate to subtype and load filtered cards', skipInCI, async ({ page }) => {
    // Start at homepage
    await page.goto('/');
  ...
  test('should preserve sidebar state after HTMX navigation', skipInCI, async ({ page }) => {
    // Start at homepage
    await page.goto('/');
