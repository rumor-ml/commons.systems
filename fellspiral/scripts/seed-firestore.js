...
if (!Array.isArray(cards)) {
  console.error('\n❌ Invalid cards.json format: expected an array');
  console.error('The file should contain a JSON array of card objects.');
  process.exit(1);
}

if (cards.length === 0) {
  console.error('\n❌ cards.json is empty');
  console.error('Run "node scripts/parse-cards.js" to regenerate it.');
  process.exit(1);
}

console.log(
  `\n📦 Seeding Firestore with ${cards.length} cards from cards.json (generated by parse-cards.js from rules.md)\n`
);
...
async function seedCards() {
  ...
  for (const card of cards) {
    // Use card.id (validated in the loop above)
    const docId = card.id;
    ...
    if (cardDoc.exists) {
      ...
      // Get existing createdAt if it exists
      const existingCreatedAt = cardDoc.data().createdAt || null;

      batch.update(cardRef, {
        ...card,
        createdAt: existingCreatedAt || card.createdAt,
