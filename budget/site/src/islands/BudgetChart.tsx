...
import { updateQualifierBreakdown } from './qualifierUtils';
...
export function BudgetChart({
  ...
  useEffect(() => {
    ...
    // - WEEKLY MODE: Single-week category comparison (vertical bar chart with budget overlay marks)
    //   Purpose: Detailed budget planning - "How am I doing this week against my budget?"
    //   Data: Single week × multiple categories, budget target bars, rollover badges
    //   Plot config: Vertical bars (Plot.barY), categorical x-axis, target overlays, emoji badges
    // - MONTHLY MODE: Multi-month time-series (stacked bar chart with trend lines)
    //   Purpose: Long-term trend analysis - "How has spending changed over time?"
    //   Data: Multiple months × multiple categories, net income line, trailing average
    //   Plot config: Stacked bars (Plot.stackY), band x-axis, dual line marks, tooltip attachment
    // Different data shapes (single week vs time series) and visual requirements (overlays vs trends)
    // prevent a unified Plot configuration. Modes use separate rendering paths.
