...
7. Configure git hooks path in the new worktree (required for pre-commit/pre-push hooks to work):
   ...
   NOTE: Git worktrees share hooks from the main repository's .git/hooks directory by default,
   but hook scripts must be written to work correctly in worktree contexts. Setting core.hooksPath
   explicitly ensures hooks execute from the main repository's hooks directory with correct paths.
   Without this configuration in some git environments, pre-commit hooks (formatting, linting)
   and pre-push hooks (tests) may fail or be skipped, leading to CI failures.
