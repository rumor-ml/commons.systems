...
echo "Starting Firebase emulators (Firestore on port ${FIRESTORE_PORT}, Storage on port ${STORAGE_PORT})..."

# Start emulators in the background
firebase emulators:start --only firestore,storage --project="${PROJECT_ID}" > /tmp/claude/emulators.log 2>&1 &
EMULATOR_PID=$!
