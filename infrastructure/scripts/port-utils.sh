...
is_lock_stale() {
  ...
  # Don't suppress stderr - we need to distinguish error types
  local kill_check_error=$(mktemp)
  if ! kill -0 "$lock_pid" 2>"$kill_check_error"; then
    local kill_exit=$?

    # Check if it's "No such process" (legitimate stale lock)
    if [ $kill_exit -eq 1 ] && grep -q "No such process" "$kill_check_error" 2>/dev/null; then
      echo "Lock holder PID $lock_pid is dead, removing stale lock" >&2
      rm -rf "$lock_dir" 2>/dev/null || true
      rm -f "$kill_check_error"
      return 0
    fi

    # Check if it's permission denied (process exists, different user)
    if grep -q "Operation not permitted" "$kill_check_error" 2>/dev/null; then
      echo "Lock holder PID $lock_pid is alive (different user), keeping lock" >&2
      rm -f "$kill_check_error"
      return 1  # Lock is valid
    fi

    # Unknown error - be conservative, keep lock
    echo "WARNING: Cannot check lock holder PID $lock_pid (exit $kill_exit)" >&2
    cat "$kill_check_error" >&2
    rm -f "$kill_check_error"
    return 1
  fi
  rm -f "$kill_check_error"

  # Process is alive and we can signal it - lock is valid
...
clear_firestore_data() {
  ...
  # Accept both 200 (data deleted) and 404 (no data to delete)
  if [[ "$status" != "200" ]] && [[ "$status" != "404" ]]; then
    echo "  ⚠️  Failed to clear Firestore data: HTTP $status" >&2
    echo "  This indicates the emulator may not be responding correctly" >&2

    # Check for specific errors
    if [[ "$status" == "500" ]] || [[ "$status" == "503" ]]; then
      echo "  Emulator returned server error - may need restart" >&2
    elif [[ "$status" == "000" ]]; then
      echo "  Network error - emulator may not be running on ${firestore_host}:${firestore_port}" >&2
    fi
  ...
  if [[ "$status" == "404" ]]; then
    echo "  ✓ Firestore already empty (nothing to clear)"
  else
...
# Note: Some apps have Firebase project suffixes (e.g., "-7696a") appended to their site IDs.
# These suffixes are auto-generated by Firebase during project creation and must match exactly.
# To find the correct site ID for a new app: check .firebaserc or firebase.json in the app directory.
# Args:
