...
test_allocate_test_ports_matches_json() {
  ...
  # TODO(#1204): Silent cleanup failures could hide filesystem issues (disk full, permissions)
  ...
  # Uses subshell to isolate port variables and prevent them from persisting in parent
  # environment, which would invalidate subsequent tests
...
test_printsync_global_setup_uses_firebase_ports() {
  ...
  # Checks for 4-digit patterns (all Firebase emulator ports) to detect configuration drift
