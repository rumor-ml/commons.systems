# {{SERVICE}}-mcp-server: MCP server for {{SERVICE_TITLE}}
#
...
buildNpmPackage {
  ...
  src = builtins.path {
    path = ../../{{SERVICE}}-mcp-server;
    name = "{{SERVICE}}-mcp-server-source";
    filter = path: type:
      let
  ...
  # Computed with: nix run nixpkgs#prefetch-npm-deps {{SERVICE}}-mcp-server/package-lock.json
  npmDepsHash = "sha256-REPLACE_ME";

  meta = with lib; {
    description = "MCP server for {{SERVICE_TITLE}}";
    homepage = "https://github.com/commons-systems/{{SERVICE}}-mcp-server";
