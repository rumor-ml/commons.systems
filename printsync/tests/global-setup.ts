...
async function globalSetup() {
  ...
  // Check if emulators are already running by testing ports (from environment)
  const authPort = parseInt(process.env.FIREBASE_AUTH_PORT || '0', 10);
  const firestorePort = parseInt(process.env.FIREBASE_FIRESTORE_PORT || '0', 10);
  const storagePort = parseInt(process.env.FIREBASE_STORAGE_PORT || '0', 10);

  if (!authPort || !firestorePort || !storagePort) {
    throw new Error('Emulator ports not configured. Run allocate-test-ports.sh first.');
  }

  const isAuthRunning = await isPortInUse(authPort);
  const isFirestoreRunning = await isPortInUse(firestorePort);
  const isStorageRunning = await isPortInUse(storagePort);
